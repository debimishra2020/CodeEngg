/*Only DBA can execute below commands or login user should require DBA privileges to run below quries.*/
...
SET AUTOTRACE ON EXPLAIN STATISTICS;  --Execute this command before running the query
...
SELECT * FROM PLAN_TABLE WHERE PLAN_ID = <>;  --Suppy the PLAN_ID for that SEL statement
...
SELECT /*+ FIRST_ROWS(10) */ * FROM emp WHERE deptno = 10;  --Only to display First 10 Rows from the Selected Table

SELECT /*+ ALL_ROWS */ * FROM emp WHERE deptno = 10;  --Returns ALL Rows from the Selected Table
...
SELECT /*+ AND_EQUAL(e,emp_dept_idx) */ * FROM emp e;   --Query can be used Multiple Indexes
...
SELECT /*+ PARALLEL_INDEX(e,emp_dept_idx , 8) */ * FROM emp e;   --Query will be used for Parallel 8 Cores using the Indexes
...
SELECT /*+ PARALLEL(8) CACHE (e) FULL (e) */  * FROM emp e;  --Query for Full execution from recent History

SELECT /*+  PARALLEL FULL (e) */  * FROM emp e;  --Query for Multiple Hints via Full

SELECT /*+ PARALLEL USE_MERGE (emp dept) */ * FROM emp, dept WHERE emp.deptno = dept.deptno;  --SORT Merge Join

SELECT /*+ PARALLEL USE_HASH (emp dept) */ * FROM emp, dept WHERE emp.deptno = dept.deptno;  --Hash  Join
